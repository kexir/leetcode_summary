...
public class Solution {
    public static List<Integer> findMinHeightTrees(int n, int[][] edges) {//57 ms
        if(n == 1){
            List<Integer> ans = new ArrayList<Integer>();
            ans.add(0);
            return ans;
        }
        ArrayList<HashSet<Integer>> neighbor = new ArrayList<HashSet<Integer>>();
        for(int i = 0; i < n; i++){
        ...
        while(n > 2) {
            ...
            for(Integer leaf : leaves) {
                int next = neighbor.get(leaf).iterator().next();
                neighbor.get(next).remove(leaf);
                if(neighbor.get(next).size() == 1){
                    temp.add(next);
                }
            }
            leaves = temp;
